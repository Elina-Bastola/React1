...
function News(props) {
    ...
    useEffect(()=>{
fetch(`https://newsapi.org/v2/top-headlines?category=${props.cat}&apiKey=f65a67959a694a7390d762df23098fa9`).then((a)=>a.json()).then((b)=>setData(b.articles))
    },[])
  return (
    <>
   {/* <img src={Logo} /> */}
...
<div className="md:flex gap-5 justify-between grid sm:grid-cols-1 md:grid-cols-3 lg:grid-cols-4 sm:mx-6 md:mx-0">
    {data.slice(5,9).map((a)=>(
        ...
        <Link to={a.url}target='_blank' className='py-3 bg-[#000] text-lime-50 text-center px-3 rounded-md'>  Read More  </Link>
        </div>
        </div>
